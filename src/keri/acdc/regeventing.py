...
Shared TEL support for ACDC v2 state registries made of 'rip' (registry
inception), 'bup' (blindable update) and 'upd' (unblindable update) events.

This module holds four closely related concerns:
...
4. The verifier-side ingest shell (Rever), which processes registry event
   streams received from others into a RegBaser with missing-anchor (.maes)
   and out-of-order (.ooes) escrows and drain methods, and vetRegistries,
   which refuses parallel-registry equivocation.

The two sides admit different grammars on purpose.  The issuer-side engine
produces and accepts only rip + bup, since upstream deprecates 'upd'.  The
verifier side (vet, vetUpdate, vetRegistries, Rever) also accepts 'upd',
because registries already issued with it must still verify; this fork keeps
that support deliberately.  The shared kernel takes the grammar as a
parameter and defaults to the narrower, issuer-side one.
...
from ..kering import (Ilks, ValidationError, ConfigurationError,
                      InvalidValueError, OutOfOrderError,
...
Fields:
    ...
    ilk (str): registry event type, one of ``rip``, ``bup`` or ``upd``
...
# Registry event grammars.  The issuer-side engine produces and accepts only
# rip + bup (upstream deprecates upd); the verifier side also accepts upd so
# that registries already issued with it still verify.
IssuerIlks = (Ilks.rip, Ilks.bup)
...
def _normalizeEventRecord(evt, *, ilks=IssuerIlks):
    """Normalize one TEL event into a parsed record

    Parameters:
        evt (SerderACDC | bytes | str): registry event material to normalize.
        ilks (tuple[str]): registry event types admitted; defaults to the
            issuer-side grammar (rip + bup).  Verifier paths admit upd as
            well, through _validateUpdate's ilks.
    ...
    Raises:
        InvalidValueError: if ``evt`` cannot be coerced into a
            ``SerderACDC``.
        ValidationError: if the event's ``ilk`` is not one of ``ilks``.
    """
    ...
    # Admit only the grammar the caller's side supports
    if ilk not in ilks:
    ...
    return _RegEventRecord(serder=serder,
                           ...
                           # for an update (bup or upd) it is carried in rd.
...
def _validateUpdate(evt, *, regid, prior=None, ilks=(Ilks.bup,)):
    """Validate one registry update against shared TEL rules.
    ...
    Parameters:
        ...
        ilks (tuple[str]): update event types admitted; defaults to the
            issuer-side ``bup`` only.  Verifier paths pass ``(bup, upd)``.

    Returns:
        _RegEventRecord: normalized facts for the validated update event.

    Raises:
        ValidationError: if the event is not one of ``ilks``.
    ...
    event = _normalizeEventRecord(evt, ilks=(Ilks.rip, *ilks))

    # Only the admitted update types belong in this path
    if event.ilk not in ilks:
        raise ValidationError(f"Expected registry update ilk in {ilks} got "
...
def vetUpdate(upd, rip):
    ...
    """
    return _validateUpdate(upd, regid=vetRip(rip).said,
                           ilks=(Ilks.bup, Ilks.upd)).serder
...
def vetRegistries(acdc, evidences, *, db, sources=None):
    ...
    Raises:
        ConflictingRegistriesError: two registries commit the ACDC with
            disagreeing states (permanent refusal)
        MisbindingError: a committing registry's bindings to the ACDC fail,
            including an ACDC issuer other than the registry's (see
            vetBindings), plus everything vet() raises for each evidence set
    """
    serder = _coerce(acdc)
    records = []
