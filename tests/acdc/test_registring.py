...
the mapping from obligation to test is auditable.  V24 (a chain mixing upd and
bup events) is absent: it tests the 'upd' event, which is deprecated.  V16b is
an added row covering an ACDC presented against a blinded head with no
disclosure.

Bakobo fork note: V17 and the Rever rows cover the two symbols this fork
carries ahead of upstream.  Everything above them is kept identical to the
upstream proposal.
"""
