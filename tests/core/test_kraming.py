...
def test_tsgs_current_when_latest_event_is_non_establishment(mockHelpingNowUTC):
    ...
    keys, so currency must be decided against ``kever.lastEst``. Deciding it against
    ``kever.sner``/``kever.serder.said`` — the latest event of any kind — makes every
    correctly formed signature look stale as soon as the sender anchors anything, since
    an interaction event advances the KEL without changing keys. That is not an edge
    case: it is every AID that has ever anchored a registry inception, a credential
    issuance or a revocation, which is every credential issuer.

    Covers both auth paths, because all four currency comparisons share the reference:
        - assk: single-key sender accepted while its latest event is an ixn
        - asmk: multi-key sender accumulates to threshold across two deliveries
    """
    salt1 = Salter(raw=b'0123456789abcdej').qb64
    salt2 = Salter(raw=b'0123456789abcdek').qb64
    salt3 = Salter(raw=b'0123456789abcdel').qb64
